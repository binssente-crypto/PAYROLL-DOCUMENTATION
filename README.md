...
## Cloud Deployment & Hybrid Sync

### Initial Cloud Onboarding
When deploying to a cloud environment (Render/Vercel) where physical hardware is unreachable:
1. Access the **Initial Setup** screen.
2. Enter **`0.0.0.0`** as the Device IP address.
3. This triggers **"Cloud Onboarding Mode"**, bypassing physical device connectivity checks and allowing you to set up your primary admin account.

### Local Bridge Agent
To sync real biometric data from your office to the cloud:
1. Copy the `bridge_agent` folder to a PC on your office network.
2. Install dependencies: `pip install pyzk requests`.
3. Set your credentials:
   ```powershell
   $env:BIZMAKER_CLOUD_URL = "https://your-backend.onrender.com"
   $env:BIZMAKER_USERNAME = "admin"
   $env:BIZMAKER_PASSWORD = "yourpassword"
   ```
4. Run `python bridge_agent.py`. The agent will automatically poll your ZKTeco machine and push logs to the cloud every 5 minutes.

## Troubleshooting

### 1. "Bad Request (400)" on Startup
- **Cause**: Django security blocking the host or missing environment variables.
- **Fix**: Ensure your Render URL is in the **`ALLOWED_HOSTS`** environment variable.
- **Fix**: Ensure **`CORS_ALLOWED_ORIGINS`** includes your Vercel URL (with `https://`).

### 2. Bridge Agent Authentication Failure
- **Cause**: Incorrect username/password or firewall blocking the API.
- **Fix**: Verify your credentials by logging into the website first.
- **Fix**: Ensure your office PC has outbound access to the Render API.

### 3. Setup Check Timed Out
- **Cause**: Backend is still booting or database connection is slow.
- **Fix**: Check Render status—ensure the service is "Live." Use `0.0.0.0` only for the initial cloud bypass.
...
- `python-dateutil`: For advanced tenure and date arithmetic.
...
# Email delivery settings (Brevo API for production)
BREVO_API_KEY=your_xkeysib_api_key_here
DEFAULT_FROM_EMAIL=BizMaker PH <your_verified_sender@example.com>
FRONTEND_BASE_URL=https://bizmaker-payroll.vercel.app
```
